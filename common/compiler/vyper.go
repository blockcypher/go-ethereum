...
func (s *Vyper) run(cmd *exec.Cmd, source string) (map[string]*Contract, error) {
	...
	return ParseVyperJSON(stdout.Bytes(), source, s.Version, s.Version,
		strings.Join(s.makeArgs(), " "))
}
...
func ParseVyperJSON(combinedJSON []byte, source string, languageVersion string,
	compilerVersion string, compilerOptions string) (map[string]*Contract, error) {
