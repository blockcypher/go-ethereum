...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (*ProcessResult, error) {
	...
	for i, tx := range block.Transactions() {
		...
		receipt, internals, vmerr, err := ApplyTransactionWithEVM(msg, gp, statedb, blockNumber, blockHash, context.Time, tx, usedGas, evm)
		if err != nil {
			return nil, fmt.Errorf("could not apply tx %d [%v]: %w", i, tx.Hash().Hex(), err)
		}
...
func ApplyTransactionWithEVM(msg *Message, gp *GasPool, statedb *state.StateDB, blockNumber *big.Int, blockHash common.Hash, blockTime uint64, tx *types.Transaction, usedGas *uint64, evm *vm.EVM) (receipt *types.Receipt, intTxs types.InternalTransactions, vmerr string, err error) {
	...
	// Set up internal transaction watcher
	...
	*usedGas += result.UsedGas
	...
	return receipt, itx.InternalTransactions(), "", nil
}
